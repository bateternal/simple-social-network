...
class LoginForm(forms.Form):
	lusername = forms.CharField(label='lusername', 
                    widget=forms.TextInput(attrs={'placeholder': 'username'}))
	lpassword = forms.CharField(label='lpassword', 
...
class Confirm(forms.Form):
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['password'].widget.attrs.update({'id': 'password_form'})		
		self.fields['bio'].widget.attrs.update({'class': 'form-control mb-2 mr-sm-2'})
		self.fields['bio'].widget.attrs.update({'id': 'bio_form'})	
